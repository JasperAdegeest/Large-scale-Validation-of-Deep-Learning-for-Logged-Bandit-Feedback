...
def run_test_set(model, test_set, batch_size):
    ...
    with torch.no_grad():
        ...
        for sample, click, propensity in BatchIterator(test_set, batch_size):
            ...
            rectified_label = torch.where((click == 0.999),
            ...
            o = torch.where((click == 0.999), not_clicked_tensor, clicked_tensor)
            N += o.sum()

            # Calculate R
            R += (rectified_label * ((1/num_candidates) / propensity).view(-1, 1)).sum()

            # Calculate C
            C += (((1/num_candidates) / propensity).view(-1, 1)).sum()
